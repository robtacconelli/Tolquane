...
- The distributed runtime (`src/tolquane/net.py`) is tested with several groups on loopback inside one test process; ports come from binding to port 0. No test needs a second machine.
- Async nodes (`src/tolquane/asyncnode.py`) are one node thread plus an event-loop thread; results come back through the node's own inbox as control entries, and `inst.in_flight` changes only via `scheduler.event()`. In the sync scheduler, `_park` waits on `inst.baton_cond`, never on a channel condition, and `_handoff` never picks the calling node with a wake-up; keep it that way or 3.14t finds the lock cycle within a few runs.
- Block workers (`tq.farm(a >> b, 4)`) expand under a scoped `_Names`; inner plain stages get `group=<farm>, role="worker"`. `optimize.py` rewrites the block tree, never the expanded graph; `launch.py` only builds and runs `tolquane run` commands. `docs/decisions/0002-fastflow-review.md` says what was taken from FastFlow and what was deferred.
- The docs site is `mkdocs build --strict` (`pip install -e ".[docs]"`); `docs/api-card.md` and `src/tolquane/ai/resources/api-card.md` must stay identical (a test checks).

...
- `docs/backlog.md` holds everything deferred. When a phase or review defers something, add it there; when it ships, move it to `CHANGELOG.md`.
- Tolquane Web (the GUI): Python side in `src/tolquane/web/`, frontend source in `web/` (Vite, React, TypeScript), built assets in `src/tolquane/web/static/` (gitignored, shipped in the wheel). Plan in `docs/web.md`, contracts in `docs/web-interfaces.md`. Node 24 is on this machine through nvm; npm's cache is set to `/mnt/1T/home/st4ck/.cache/npm`; run Playwright with `PLAYWRIGHT_BROWSERS_PATH=/mnt/1T/home/st4ck/.cache/ms-playwright`. The root disk is nearly full: nothing goes under `~` or `/tmp` beyond the session scratchpad.
